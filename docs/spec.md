...
1. First, calculate the hash of [WithdrawArgs](#withdrawargs) with position `public_key`:
   [get\_message\_hash](#get-message-hash) on [WithdrawArgs](#withdrawargs) with position `public_key`.

2. Then, calculate the hash of [ForcedWithdrawArgs](#forcedwithdrawargs) with position `public_key`:
   [get\_message\_hash](#get-message-hash) on [ForcedWithdrawArgs](#forcedwithdrawargs) with position `public_key`.
...
1. Position exists.
2. The forced withdraw request exists and is consumed.
...
1. Calculate the hash of [ForcedTrade](#forcedtrade) with position A `public_key`:
   [get\_message\_hash](#get-message-hash) on [ForcedTrade](#forcedtrade) with position A `public_key`.
2. Calculate the hash of [ForcedTrade](#forcedtrade) with position B `public_key`:
   [get\_message\_hash](#get-message-hash) on [ForcedTrade](#forcedtrade) with position B `public_key`.
...
3. Transfer `premium_cost` (forced fee) from the caller to the sequencer address.
...
1. Position A exists.
2. Position B exists.
3. The forced trade request exists and is consumed.
