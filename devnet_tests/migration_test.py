...
async def test_migration(test_utils: PerpetualsTestUtils):
    ...
    abi, cairo_version = await ContractAbiResolver(
        ...
        client=test_utils.known_accounts["deployer"].client,
        proxy_config=ProxyConfig(),
    ).resolve()
    vault_contract = Contract(
        address=new_vault_deploy_result.deployed_contract.address,
        abi=abi,
        provider=test_utils.known_accounts["deployer"],
        cairo_version=cairo_version,
    )
    await setup_upgrade_roles(test_utils, vault_contract)
    await test_utils.upgrade_contract(
        new_class_hash=new_vault_declare_result.class_hash, contract=vault_contract, eic_data=None
    )


async def setup_upgrade_roles(test_utils: PerpetualsTestUtils, contract: Contract):
    contract_address = contract.address
    abi = contract.data.abi

    governance_admin_contract = Contract(
        address=contract_address,
        abi=abi,
        provider=test_utils.known_accounts["governance_admin"],
        cairo_version=contract.data.cairo_version,
    )

    invocation = await governance_admin_contract.functions["register_upgrade_governor"].invoke_v3(
        test_utils.known_accounts["deployer"].address,
        auto_estimate=True,
    )
