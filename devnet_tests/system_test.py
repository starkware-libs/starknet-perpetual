...
async def test_trade(test_utils: PerpetualsTestUtils):
    ...
    base_amount = 37
    quote_amount = 5303580
    fee_amount = 0
    expiration = NOW + 1000000000
    order_a = await test_utils.create_order(
        position_id_a, base_asset_id, base_amount, -quote_amount, fee_amount, expiration
    )
    order_b = await test_utils.create_order(
        position_id_b, base_asset_id, -base_amount, quote_amount, fee_amount, expiration
    )
