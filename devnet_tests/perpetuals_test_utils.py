...
class PerpetualsTestUtils:
    def __init__(
        ...
        self.perpetuals_contract_class_hash = None
    ...
    async def process_deposit(
        ...
        amount: int,
        salt: int,
        non_base_collateral_args: Optional[Tuple[int, int]] = None,
    ):
    ...
    async def upgrade_perpetuals_contract(
        ...
        try:
            ...
            if (
                "Class already declared" in str(e)
                and self.perpetuals_contract_class_hash is not None
            ):
                new_class_hash = self.perpetuals_contract_class_hash
            else:
                raise e
        self.perpetuals_contract_class_hash = new_class_hash
    ...
    async def invest_in_vault(self, account: Account, min_base_amount: int, quote_amount: int):
        ...
        order = await self.create_limit_order(
            user_position_id, VAULT_POSITION_ID, min_base_amount, quote_amount
        )
        ...
        await self.process_deposit(
            account,
            minted_shares,
            order[SALT_STR],
            (VAULT_ASSET_ID, self.perpetuals_contract_address),
        )
    ...
    async def redeem_from_vault(self, account: Account, base_amount: int, min_quote_amount: int):
        ...
        user_order = await self.create_limit_order(
            user_position_id, VAULT_POSITION_ID, base_amount, min_quote_amount
        )
        vault_approval = await self.create_limit_order(
            VAULT_POSITION_ID, user_position_id, -base_amount, -min_quote_amount
        )
