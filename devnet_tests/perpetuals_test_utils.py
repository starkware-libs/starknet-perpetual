...
class PerpetualsTestUtils:
    ...
    async def get_asset_timely_data(self, asset_id: int) -> dict:
        (asset_timely_data,) = await self.operator_contract.functions["get_asset_timely_data"].call(
    ...
    async def new_position(self, account: Account, seed: int = 0) -> int:
        error = None
        random.seed(seed)
        tries = 3
        while tries > 0:
        ...
        async def _get_all_funding_indices() -> Dict[int, int]:
            async def fetch_funding_index(asset_id: int) -> Tuple[int, Optional[int]]:
                try:
                    ...
                    funding_index_value = timely_data["funding_index"]["value"]
        ...
        for asset_id in self.asset_ids:
            if asset_id in funding_ticks_diffs.keys():
                new_funding_indices.append(
                    {
                        "asset_id": formatted_asset_id(asset_id),
                        "funding_index": formatted_funding_index(
                ...
                new_funding_indices.append(
                    {
                        "asset_id": formatted_asset_id(asset_id),
                        "funding_index": formatted_funding_index(current_funding_indices[asset_id]),
                    }
                )
