...
from conftest import NOW, EXTENDED_SYNTHETIC_ASSET_IDS, contracts_inner_fixture
from typing import Union
from devnet_tests.conftest import resource_bounds
...
class PerpetualsTestUtils:
    def __init__(
        ...
        self.new_account_key_pairs = {}  # key_pairs[account] = (public_key, private_key)
    ...
    async def new_account(self) -> Account:
        ...
        # Use the perpetuals contract address from known_contracts
        perpetuals_contract_address = self.known_contracts["operator"].address
    ...
    async def deposit(self, account: Account, amount: int):
        ...
        async def _approve_deposit(account: Account, amount: int):
            ...
            # Approve the perpetuals contract (not the operator wallet) to spend tokens
            perpetuals_contract_address = self.known_contracts["operator"].address
            invocation = await erc20_contract.functions["approve"].invoke_v3(
                perpetuals_contract_address, amount, auto_estimate=True
            )
            await invocation.wait_for_acceptance(check_interval=0.1)
    ...
    async def upgrade_contract(
        ...
        is_perpetuals_contract = False
        if contract is None:
            is_perpetuals_contract = True
            contract = self.known_contracts["deployer"]

        invocation = await contract.functions["add_new_implementation"].invoke_v3(
            {
        ...
        invocation = await contract.functions["replace_to"].invoke_v3(
            {
                "impl_hash": new_class_hash,
                "eic_data": eic_data,
                "final": False,
        ...
        if is_perpetuals_contract:
            contract_address = contract.address
            self.known_contracts = await contracts_inner_fixture(
                contract_address, self.known_accounts
            )
    ...
    async def register_and_activate_external_component(
        ...
        component_declare_result = await declare_contract(
            contract_name, self.known_accounts["deployer"]
        )
        invocation = (
            await self.known_contracts["deployer"]
