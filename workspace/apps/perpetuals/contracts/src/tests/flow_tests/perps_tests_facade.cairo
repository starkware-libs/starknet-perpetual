...
use perpetuals::tests::event_test_utils::{
    assert_add_synthetic_event_with_expected, assert_add_vault_share_event_with_expected,
    ...
    assert_register_vault_event_with_expected, assert_trade_event_with_expected,
...
pub impl PerpsTestsFacadeImpl of PerpsTestsFacadeTrait {
    ...
    fn add_vault_collateral_asset(
        ...
        self.set_app_governor_as_caller();
        let asset_dispatcher = IAssetsDispatcher { contract_address: self.perpetuals_contract };
        asset_dispatcher
            .add_vault_collateral_asset(
                asset_id: *vault_asset_info.asset_id,
                erc20_contract_address: *vault_asset_info.vault_contract_address,
        ...
        assert_add_vault_share_event_with_expected(
            spied_event: self.get_last_event(contract_address: self.perpetuals_contract),
            asset_id: *vault_asset_info.asset_id,
            risk_factor_tiers: *vault_asset_info.risk_factor_data.tiers,
            risk_factor_first_tier_boundary: *vault_asset_info.risk_factor_data.first_tier_boundary,
            risk_factor_tier_size: *vault_asset_info.risk_factor_data.tier_size,
            resolution_factor: *vault_asset_info.resolution_factor,
            quorum: vault_asset_info.oracles.len().try_into().unwrap(),
            erc20_contract_address: *vault_asset_info.vault_contract_address,
        ...
        assert_eq!(
            asset_dispatcher.get_asset_config(asset_id: *vault_asset_info.asset_id).status,
            AssetStatus::PENDING,
        );
        ...
        let account = vault_asset_info.vault.account;
        let request_hash = RegisterVaultArgs {
        ...
        assert_register_vault_event_with_expected(
            spied_event: self.get_last_event(contract_address: self.perpetuals_contract),
            vault_position_id: *vault_asset_info.vault_position_id,
            vault_contract_address: *vault_asset_info.vault_contract_address,
            vault_asset_id: *vault_asset_info.asset_id,
        ...
        // Add oracle.
        for oracle in vault_asset_info.oracles {
            self.set_app_governor_as_caller();
            asset_dispatcher
                .add_oracle_to_asset(
                    *vault_asset_info.asset_id,
                    *oracle.key_pair.public_key,
                    *oracle.name,
                    *vault_asset_info.asset_name,
                );
        }
