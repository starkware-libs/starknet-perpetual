...
pub impl PerpsTestsFacadeImpl of PerpsTestsFacadeTrait {
    ...
    fn multi_trade(ref self: PerpsTestsFacade, trades: Span<Settlement>) {
        ...
        for trade in trades {
            ...
            let mut i = 0;
            loop {
                if i >= position_data_a.synthetics.len() {
                    break;
                }
                let synthetic = *position_data_a.synthetics[i];
            ...
            let updated_position_data = PositionData {
            ...
            positions_dict
                .insert(
                    ...
                    NullableTrait::new(updated_position_data),
                );
            ...
            let mut i = 0;
            loop {
                if i >= position_data_b.synthetics.len() {
                    break;
                }
                let synthetic = *position_data_b.synthetics[i];
                if synthetic.id == asset_id {
                    let new_balance = synthetic.balance - settlement.actual_amount_base_a.into();
                    new_synthetics_b
                        .append(
            ...
            let updated_position_data_b = PositionData {
