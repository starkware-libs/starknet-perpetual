...
fn test_spot_collateral_deposit_buy_synthetic_transfer_then_withdraw_fails() {
    ...
    // Deposit 1 BTC spot collateral to user_1 (using 2 quantized units to represent 1 BTC).
    // This allows us to transfer 1 unit (0.5 BTC) later.
    let deposit_info_btc = state
        .facade
        .deposit_spot(
            depositor: user_1.account,
    ...
    // user_1 buys 1 synthetic for 100 (quote) + 5 (fee) = 105 total cost
    // This will make base collateral negative: 100 - 105 = -5
    // Total value remains similar: BTC value (200) + synthetic value (100) - costs = ~295
    let order_user_1 = state
        .facade
        .create_order(
            user: user_1,
    ...
    // Position now has: negative base collateral (-5), BTC spot (2), synthetic (1)
    state
        .facade
        .trade(
            order_info_a: order_user_1,
    ...
    // Transfer 0.5 BTC (1 unit) - should succeed.
    let transfer_info = state
        .facade
    ...
    // User_1 still has 0.5 BTC remaining (1 unit).
    assert_eq!(
        state.facade.get_position_asset_balance(user_1.position_id, asset_id_btc), 1_u64.into(),
    );

    // Now try to withdraw the remaining 0.5 BTC (1 unit) - should fail because position would
    // become unhealthy.
