...
fn test_withdraw_spot_collateral_negative_balance() {
    // Test that withdrawing more spot collateral than available fails with proper error.
    // This test verifies the bug fix that prevents negative spot balances on withdrawal.
    // Setup.
    let risk_factor_data = RiskFactorTiers {
        tiers: array![10].span(), first_tier_boundary: MAX_U128, tier_size: 1,
    };
    ...
    // POSITION_SPOT_BALANCE_NEGATIVE.
