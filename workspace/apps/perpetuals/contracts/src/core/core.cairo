...
pub mod Core {
    ...
    use perpetuals::core::errors::{
        AMOUNT_OVERFLOW, FORCED_WAIT_REQUIRED, INVALID_ZERO_TIMEOUT, SYNTHETIC_IS_ACTIVE,
    ...
    use starkware_utils::signature::stark::{PublicKey, Signature, validate_stark_signature};
    ...
    pub impl CoreImpl of ICore<ContractState> {
        ...
        fn forced_trade_request(
            ...
            validate_stark_signature(
                public_key: public_key_b, msg_hash: hash, signature: signature_b,
            );
            ...
            self
                .emit(
                    events::ForcedTrade {
                        ...
                        actual_amount_quote_a: order_b.base_amount,
                        hash,
                    },
                );
        }
