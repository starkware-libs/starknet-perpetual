...
pub mod Core {
    ...
    pub impl InternalCoreFunctions of InternalCoreFunctionsTrait {
        ...
        fn _withdraw_from_vault_contract(
            ...
            let unquantized_vault_share_received_amount: u256 =
                (quantized_vault_share_received_amount
                .abs()
                * quantum.into())
                .into();
            erc20_vault_dispatcher
                .approve(spender: vault_address, amount: unquantized_vault_share_received_amount);
            let unquantized_amount = IProtocolVaultDispatcher { contract_address: vault_address }
                .redeem_with_price(
                    shares: (quantized_vault_share_received_amount.abs()).into(),
                    value_of_shares_in_assets: unquantized_vault_share_received_amount,
                );

            assert(
                unquantized_amount == unquantized_vault_share_received_amount,
                SHARES_BALANCE_MISMATCH,
            );
            ...
            assert(
                after_withdraw_shares_balance == before_withdraw_shares_balance
                    - unquantized_vault_share_received_amount,
                SHARES_BALANCE_MISMATCH,
            );
