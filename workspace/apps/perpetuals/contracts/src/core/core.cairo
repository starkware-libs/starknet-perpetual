...
pub mod Core {
    ...
    pub impl CoreImpl of ICore<ContractState> {
        ...
        fn multi_trade(ref self: ContractState, operator_nonce: u64, trades: Span<Settlement>) {
            ...
            let mut cache: Felt252Dict<Nullable<PositionTVTR>> = Default::default();
            ...
            for _trade in trades {
                ...
                let pre_trade_state_a = cache.get(position_id_a);
                let pre_trade_state_b = cache.get(position_id_b);
                let (updated_a, updated_b) = self
                    ._execute_trade(
                        ...
                        tvtr_a_before: pre_trade_state_a,
                        tvtr_b_before: pre_trade_state_b,
                    );
                cache.insert(position_id_a, NullableTrait::new(updated_a));
                cache.insert(position_id_b, NullableTrait::new(updated_b));
            }
        }
