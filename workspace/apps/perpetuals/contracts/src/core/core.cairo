...
pub mod Core {
    ...
    pub impl CoreImpl of ICore<ContractState> {
        ...
        fn forced_withdraw_request(
            ...
            // Note: Caller must be extracted here as a workaround for a snforge bug
            // that affects caller address retrieval in library calls.
            let caller = get_caller_address();
            let dispatcher = self.external_components._get_forced_requests_manager_dispatcher();
            dispatcher
        ...
        fn forced_trade_request(
            ...
            let dispatcher = self.external_components._get_forced_requests_manager_dispatcher();
            dispatcher
                .forced_trade_request(:caller, :signature_a, :signature_b, :order_a, :order_b);
        }
        ...
        fn forced_redeem_from_vault_request(
            ...
            dispatcher
                .forced_redeem_from_vault_request(
                    :caller, :signature, :vault_signature, :order, :vault_approval,
                );
        }
