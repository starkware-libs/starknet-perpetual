...
pub mod Core {
    ...
    use perpetuals::core::errors::{
        ASSET_ID_NOT_COLLATERAL, ASSET_NOT_EXISTS, CANT_LIQUIDATE_IF_POSITION,
        ...
        fn enrich_asset(
            ...
            let asset_enriched = if let Option::Some((asset_id, diff)) = position_diff.asset_diff {
                let balance_before = self
                    .positions
                    .get_asset_balance(:position, asset_id: asset_id);
                let balance_after = balance_before + diff;
                let price = self.assets.get_asset_price(asset_id: asset_id);
                let risk_factor_before = self
                    .assets
                    .get_asset_risk_factor(asset_id, balance_before, price);
                let risk_factor_after = self
                    .assets
                    .get_asset_risk_factor(asset_id, balance_after, price);
            ...
            AssetEnrichedPositionDiff {
                collateral_diff: position_diff.collateral_diff, asset_enriched: asset_enriched,
            }
        }
