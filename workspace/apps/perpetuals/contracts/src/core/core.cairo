...
pub mod Core {
    ...
    pub impl InternalCoreFunctions of InternalCoreFunctionsTrait {
        ...
        fn _withdraw_from_vault_contract(
            ...
            let unquantized_amount = IERC4626Dispatcher { contract_address: vault_address }
                .redeem(
            ...
            // Validate balances to ensure correctness
            assert(
                after_withdraw_balance == before_withdraw_balance + unquantized_amount,
                COLLATERAL_BALANCE_MISMATCH,
            );
            assert(
                after_withdraw_shares_balance == before_withdraw_shares_balance
                    - unquantized_shares_amount,
                SHARES_BALANCE_MISMATCH,
            );
