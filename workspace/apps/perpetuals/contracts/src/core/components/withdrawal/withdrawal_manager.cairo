...
pub(crate) mod WithdrawalManager {
    ...
    use perpetuals::core::types::asset::AssetId;
    use perpetuals::core::types::position::{PositionId, Position, PositionTrait};
    use starknet::storage::{StoragePointerReadAccess, StoragePath};
    ...
    impl WithdrawalManagerImpl of IWithdrawalManager<ContractState> {
        ...
        fn forced_withdraw_request(
            ...
            // Transfer premium_cost (forced fee) from position to fee position
            let premium_cost = self.premium_cost.read();
            let token_contract = self.assets.get_collateral_token_contract();
            assert(
                token_contract
                    .transfer(
                        recipient: get_block_info().sequencer_address, amount: premium_cost.into(),
                    ),
                TRANSFER_FAILED,
            );
        ...
        fn forced_withdraw(
            ...
            let forced_withdraw_request_hash = forced_withdraw_args.get_message_hash(:public_key);
            let request_time = self
                .request_approvals
                .get_forced_request_time(forced_withdraw_request_hash);
    ...
    impl PrivateImpl of PrivateTrait {
        fn _withdraw(
            ...
            let hash = self.request_approvals.consume_approved_request(
                    args: WithdrawArgs {
                        position_id, salt, expiration, collateral_id, amount, recipient,
                    },
                    public_key: position.get_owner_public_key(),
