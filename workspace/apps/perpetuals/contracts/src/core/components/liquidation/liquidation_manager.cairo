...
pub(crate) mod LiquidationManager {
    ...
    use crate::core::errors::{
        CANT_LIQUIDATE_IF_POSITION, CANT_TRADE_WITH_FEE_POSITION, CANT_TRADE_WITH_SAME_POSITION,
    };
    ...
    impl LiquidationManagerImpl of ILiquidationManager<ContractState> {
        ...
        fn liquidate_spot_asset(
            ...
            assert(
                liquidator_order.receive_position != liquidated_position_id,
                CANT_TRADE_WITH_SAME_POSITION,
            );
    ...
    pub impl InternalFunctions of LiquidationManagerFunctionsTrait {
        ...
        fn _execute_liquidate<T, +ValidateableOrderTrait<T>, +Drop<T>, +Copy<T>>(
            ...
            let mut liquidator_position_diff = PositionDiff {
                collateral_diff: -liquidated_order.quote_amount().into()
                    - actual_liquidator_fee.into(),
                asset_diff: Option::None,
            };
            let mut liquidator_receive_position_diff = PositionDiff {
                collateral_diff: Zero::zero(), asset_diff: Option::None,
            };
            if liquidator_order.receive_position() != liquidator_order.position_id() {
                liquidator_receive_position_diff
                    .asset_diff =
                        Option::Some(
                            (
