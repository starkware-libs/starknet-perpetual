    ...
    impl AssetsManagerImpl of IAssetsExternal<ContractState> {
        ...
        fn update_asset_risk_factor(
            ...
            if is_increasing {
                ...
                assert(request_asset_id == asset_id, RF_INCREASE_REQUEST_NOT_FOUND);
                ...
                assert(request_tiers.len() == risk_factor_tiers.len().into(), RF_REQUEST_MISMATCH);

                // Verify each tier matches
                for i in 0..risk_factor_tiers.len() {
                    let requested_tier = request_tiers[i.into()].read();
                    assert(requested_tier == *risk_factor_tiers[i], RF_REQUEST_MISMATCH);
                }
