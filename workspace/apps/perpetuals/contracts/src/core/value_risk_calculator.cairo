...
mod tests {
    ...
    fn test_calculate_max_allowed_change_large_values() {
        // Test with large values: |1000000| * 86400 * 2^32 / 2^32
        // = 1000000 * 86400 * 4294967296 / 4294967296
        // = 1000000 * 86400 = 86400000000
        let pnl: i64 = 1_000_000;
        let time_diff: u64 = 86_400; // 1 day in seconds
        let max_interest_rate_per_sec: u32 = INTEREST_RATE_SCALE.try_into().unwrap();
        let result = calculate_max_allowed_change(pnl, time_diff, max_interest_rate_per_sec);
        // Expected: (1000000 * 86400 * 2^32) / 2^32 = 1000000 * 86400 = 86400000000
        assert_eq!(result, 86_400_000_000);
    }
    ...
    fn test_calculate_max_allowed_change_one_hour() {
        ...
        // ≈ 4194.30 (floor division = 4194)
        let pnl: i64 = 10_000;
        let time_diff: u64 = 3_600; // 1 hour
        let max_interest_rate_per_sec: u32 = 500_000;
        let result = calculate_max_allowed_change(pnl, time_diff, max_interest_rate_per_sec);
        // Expected: (10000 * 3600 * 500000) / 2^32 = 18000000000000 / 4294967296 = 4194
        assert_eq!(result, 4194);
    }
    ...
    fn test_calculate_max_allowed_change_min_i64_pnl() {
        // Test with minimum i64 PnL (should use abs): |-9223372036854775808| * 1 * 1 / 2^32
        // Note: abs(i64::MIN) = i64::MAX + 1, which overflows i64, but abs() should handle it
        // This tests edge case of negative minimum value
        let pnl: i64 = -9_223_372_036_854_775_808_i64; // i64::MIN
        let time_diff: u64 = 1;
        let max_interest_rate_per_sec: u32 = 1;
        // This should not panic and should calculate correctly
        let result = calculate_max_allowed_change(pnl, time_diff, max_interest_rate_per_sec);
        // The result should be the same as i64::MAX case since we use abs
        // abs(i64::MIN) = 9223372036854775808, which is i64::MAX + 1
        // But when converted to u128, it becomes 9223372036854775808
        // Expected: (9223372036854775808 * 1 * 1) / 2^32 = 9223372036854775808 / 4294967296
        // = 2147483648 (floor division)
