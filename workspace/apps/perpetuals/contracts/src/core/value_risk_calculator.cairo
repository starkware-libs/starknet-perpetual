...
pub fn calculate_position_tvtr_change(
    ...
    if let Option::Some(asset_diff) = synthetic_enriched_position_diff.synthetic_enriched {
        ...
        let asset_value_before = asset_diff.price.mul_and_div_price_scale(rhs: asset_diff.balance_before);
        let asset_value_after = asset_diff.price.mul_and_div_price_scale(rhs: asset_diff.balance_after);
    ...
    // asset_value is in units of 10^-6 USD.
    total_value_after += price.mul_and_div_price_scale(rhs: synthetic_enriched_position_diff.collateral_diff);
...
pub fn calculate_position_tvtr_before(
    ...
    for synthetic in unchanged_synthetics {
        // asset_value is in units of 10^-6 USD.
        let asset_value: i128 = (*synthetic.price).mul_and_div_price_scale(rhs: *synthetic.balance);
    ...
    // PRICE_SCALE.
    let price: Price = One::one();
    total_value += price.mul_and_div_price_scale(rhs: position_diff_enriched.collateral_enriched.before);
...
mod tests {
    ...
    fn test_calculate_position_tvtr_before_with_multiple_assets() {
        ...
        // + balance_3 * PRICE_3()  + collateral_balance_before  = 50 * 900 + 100 * 1000  + 150 * 500
        //+ 50 = 220_050
